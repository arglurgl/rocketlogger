...
async def run(addr, do_arm, do_transfer, do_stop, outfile):
    if not addr:
    ...
    def handle_notify(sender, data):
        nonlocal in_log, received, buffer, transfer_done
...
def main():
    args = parse_args()
    asyncio.run(run(args.addr, args.arm, args.transfer, args.stop, args.outfile))
